...
__global__ void process_smooth(realw_const_p xstore_me,realw_const_p zstore_me,realw_const_p xstore_other,realw_const_p zstore_other, realw_const_p data_other, const realw sigma_h2_inv, const realw sigma_v2_inv,const realw taille_domaine,const realw dist_bord, const int iker, const int nspec_me, const int nspec_other, const realw v_criterion, const realw h_criterion, realw_const_p jacobian, realw_p sum_data_smooth, realw_p normalisation,realw_const_p wgll_sq){
 ...
 sh_test[threadIdx.x]= ( (x_me < dist_bord) || (z_me < dist_bord) ||( x_me > taille_domaine - dist_bord) ||( z_me > taille_domaine - dist_bord)) ? 1 : 0 ;
    __syncthreads();
test=0;
for (int k=0;k<NGLL2;k++) test+=sh_test[k];
...
dat=data_other[NGLL2*ispec + igll ];
normalisation_slice=1;
}
