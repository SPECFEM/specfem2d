...
program smooth_stations
  ...
  integer :: iproc
    ...
    real(kind=CUSTOM_REAL):: taille_domaine,dist_bord
  ...
  if (command_argument_count() /= NARGS) then
    if (myrank == 0) then
        print *, 'USAGE:  mpirun -np NPROC bin/xsmooth_stations NSPEC KERNEL_NAME INPUT_DIR OUPUT_DIR '
  ...
  read(arg(1),*) taille_domaine
  read(arg(2),*) dist_bord
...
! each process reads all the other slices and Gaussian filters the values
  ...
  do iproc = 0,NPROC-1
    ...
    call compute_smooth(Container,jacobian,xstore_other,zstore_other,dat_store,nspec_other,taille_domaine,dist_bord)
