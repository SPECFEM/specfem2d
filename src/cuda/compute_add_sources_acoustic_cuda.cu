  ...
  if( isource < NSOURCES ){
    ...
    if(myrank == islice_selected_source[isource]) {
      ...
      if(ispec_is_inner[ispec] == phase_is_inner && ispec_is_acoustic[ispec] ) {
        ...
        stf = source_time_function[INDEX2(nsources_local,num_source_locale,it)];

        kappal = kappastore[INDEX3(NGLLX,NGLLX,i,j,ispec)];

        atomicAdd(&potential_dot_dot_acoustic[iglob],
                  -sourcearrays[INDEX4(NSOURCES,NDIM,NGLLX,isource, 0,i,j)]*stf);
  ...
  if(irec_local < nadj_rec_local) {
    ...
    if( ispec_is_acoustic[ispec] ){
      ...
      if(ispec_is_inner[ispec] == phase_is_inner) {
        ...
        realw  kappal = kappastore[INDEX3(5,5,i,j,ispec)];
