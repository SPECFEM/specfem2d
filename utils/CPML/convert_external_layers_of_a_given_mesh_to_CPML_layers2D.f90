...
! adapted by Andrea Colombi from the 3D version to make it work in 2D, February 2015

! convert outer layers of an existing CUBIT 2D mesh file stored in SPECFEM2D format to CPML layers,
! i.e., create a 'PML_elements' file for that existing mesh
...
! flags for the seven CPML regions
  ...
  print *,'The thickness can be different between the X and Z sides. But for X it must not vary,'
  print *,'and for Y it must not vary'
...
! open SPECFEM3D_Cartesian mesh file to read the points
    open(unit=23,file='mesh/nodes_coords_file',status='old',action='read')
    read(23,*) npoin
    allocate(x(npoin))
    allocate(z(npoin))
    do ipoin = 1,npoin
...
! open SPECFEM3D_Cartesian topology file to read the mesh elements
  open(unit=23,file='mesh/mesh_file',status='old',action='read')
  read(23,*) nspec
...
!************* generate the CPML database file *************
  ...
  open(unit=24,file='mesh/PML_elements',status='unknown',action='write')
...
! write the CPML flag for each CPML element
  ...
  print *
  print *,'CPML absorbing layer file "PML_elements" has been successfully created'
  print *

!!!!! Andrea Colombi: This is easy to achieve using CUBIT scripts, thus I comment it out here
