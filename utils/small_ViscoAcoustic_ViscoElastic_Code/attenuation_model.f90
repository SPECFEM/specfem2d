...
subroutine viscoelastic_attenuation_model()
  ...
  call compute_attenuation_coeffs(N_SLS,QKappa,f0,f_min_attenuation,f_max_attenuation, &
       tau_epsilon_nu1_d,tau_sigma_nu1)


  call compute_attenuation_coeffs(N_SLS,QMu,f0,f_min_attenuation,f_max_attenuation, &
       tau_epsilon_nu2_d,tau_sigma_nu2)


!  print *
!  print *,'N_SLS, QKappa, QMu = ',N_SLS, QKappa, QMu
...
!  print *
  ...
  tau_epsilon_nu2(:) = real(tau_epsilon_nu2_d(:),kind=4)

  inv_tau_sigma_nu1_sent(:) = real(dble(1.d0) / tau_sigma_nu1(:),kind=4)
  inv_tau_sigma_nu2_sent(:) = real(dble(1.d0) / tau_sigma_nu2(:),kind=4)
  ...
  phi_nu2_sent(:) = real((dble(1.d0) - tau_epsilon_nu2_d(:)/tau_sigma_nu2(:)) / tau_sigma_nu2(:) &
       / sum(tau_epsilon_nu2_d/tau_sigma_nu2),kind=4)
...
end subroutine viscoelastic_attenuation_model


subroutine viscoacoustic_attenuation_model()
  ...
  ! attenuation constants for standard linear solids
  ! nu1 is the dilatation/incompressibility mode (QKappa)
  ! nu2 is null the shear mode (Qmu)
  ! array index (1) is the first standard linear solid, (2) is the second etc.
...
!  print *
  ...
  if (any(tau_sigma_nu1 < 0.d0) .or. &
       any(tau_epsilon_nu1_d < 0.d0)) &
       stop 'error: negative relaxation time found for a viscoelastic material'
  ...
  if (minval(tau_epsilon_nu1_d / tau_sigma_nu1) < 0.999d0) then
     ...
     print *,'*******************************************************************************'
     print *,'minval(tau_epsilon_nu1 / tau_sigma_nu1) = ',minval(tau_epsilon_nu1_d / tau_sigma_nu1)
     print *,'WARNING: tau_epsilon should never be smaller than tau_sigma for viscoaacoustic'
     print *,'*******************************************************************************'
     print *,'*******************************************************************************'
     print *,'*******************************************************************************'
     print *,'*******************************************************************************'
  ...
  if (Mu_nu1_sent < 1.) &
       stop 'error Mu_nu1 must be greater than one'

end subroutine viscoacoustic_attenuation_model
