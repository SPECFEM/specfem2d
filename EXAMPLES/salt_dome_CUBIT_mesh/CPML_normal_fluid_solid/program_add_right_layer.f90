...
integer, parameter :: nspec_to_add = 101
...
double precision, dimension(npoin_to_add) :: x_right_edge,z_right_edge
double precision, dimension(npoin_to_add) :: x_right_edge_sorted,z_right_edge_sorted
integer, dimension(npoin_to_add) :: ibool_right_edge,ibool_right_edge_copy,ibool_right_edge_sorted
...
open(unit=27,file='right_edge',status='old')
do i = 1,npoin_to_add
 read(27,*) ibool_right_edge(i)
 x_right_edge(i) = x(ibool_right_edge(i))
 z_right_edge(i) = z(ibool_right_edge(i))
enddo
close(27)

print *,'minval maxval x_right_edge = ',minval(x_right_edge),maxval(x_right_edge)
print *,'minval maxval z_right_edge = ',minval(z_right_edge),maxval(z_right_edge)

! loop on the z value from smallest to highest (inefficient bubble sort)
ibool_right_edge_copy(:) = ibool_right_edge(:)
...
enddo
 x_right_edge_sorted(i2) = x_right_edge(i3)
 z_right_edge_sorted(i2) = z_right_edge(i3)
 ibool_right_edge_sorted(i2) = ibool_right_edge(i3)
 ibool_right_edge_copy(i3) = -1 !! mark as already used

print *,i2,x_right_edge_sorted(i2),z_right_edge_sorted(i2)
enddo
...
do i = 1,NPML
do i2 = 1,npoin_to_add
print *,x_right_edge_sorted(i2) + thickness_PML*i,z_right_edge_sorted(i2)
enddo
enddo
print *
...
if (i == 1) then !! link first PML layer to the existing mesh
  print *,ibool_right_edge_sorted(ispec2), npoin + ispec2, npoin + ispec2 + 1,ibool_right_edge_sorted(ispec2+1)
...
print *
print *,'in absorbing edge list, replace all lines that end with '\0' 2'\0' with this:'
ispec = nspec
do i = 1,NPML
do ispec2 = 1,nspec_to_add
  ispec = ispec + 1
if (i == NPML) then
  print *,ispec,' 2 ',npoin + ispec2 + (i-1)*npoin_to_add, npoin + ispec2 + (i-1)*npoin_to_add + 1,' 2'
endif
enddo
enddo
print *
...
if (ispec2 == 1) then
if (i == 1) then !! link first PML layer to the existing mesh
  print *,ispec,' 2 ', ibool_right_edge_sorted(ispec2), npoin + ispec2, ' 1'
else !! add nPML-1 more PML layers
  print *,ispec,' 2 ', npoin + ispec2 + (i-2)*npoin_to_add, npoin + ispec2 + (i-1)*npoin_to_add, ' 1'
endif
endif
enddo
enddo
...
!  print *,ispec,8
!else
  print *,ispec,2  ! code for a pure CPML layer outside a corner
!endif
enddo
enddo
print *

end program add_right_PML
