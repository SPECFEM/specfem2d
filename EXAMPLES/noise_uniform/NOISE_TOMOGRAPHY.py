...
def NOISE_TOMOGRAPHY(NSTEP=None,dt=None,Tmin=None,Tmax=None,NOISE_MODEL=None,show_figures=True):
    ...
    ***NEVER*** try to calculate "NSTEP" and "dt" by yourself!
    ...
    ## derived parameters
    T = (NSTEP - 1) * dt              # total duration
    N_mid = int((NSTEP + 1) / 2)      # mid point
    fmax = 1.0 / 2.0 / dt             # Nyquist frequency
    df = 1.0 / T                      # frequency increment
    ...
    # frequencies
    f_pos = arange(0.0,fmax+df,df)
    f_neg = arange(-fmax,-df+df,df)
    f = np.concatenate([f_pos,f_neg])
    #print(f,len(f))

    # check
    if len(f) != NSTEP:
        print("Error: number of frequencies {} should match number of steps {}".format(len(f),NSTEP))
        print("Please check that number of steps is odd for generating noise source S_squared")
        sys.exit(1)
    ...
    print('')
    print('For noise simulations, S_squared should be put into directory:')
    print('./NOISE_TOMOGRAPHY/')
    print('*************************************************************')
    print('')
...
if __name__ == '__main__':
    ...
    if len(sys.argv) < 6:
        ...
        #DT = float(sys.argv[2])
        # note: conversion float() would not work with the fortran scientific format for double precision, like e.g., 5.d-2,
        #       but it recognizes 5.e-2
        tmp = sys.argv[2].replace("d","e")    # 5.d-2 -> 5.e-2
        DT = float(tmp)
        #print("DT: ",DT,sys.argv[2],tmp)
